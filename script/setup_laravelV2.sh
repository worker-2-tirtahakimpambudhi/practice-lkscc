...
read -p "Enter Folder name (default: lksdiycc2024): " input_repo_name
...
# Copy Laravel project from parent directory including hidden files
if [ -d "../lksdiycc2024" ]; then
    mkdir -p /var/www/lksdiycc2024 || handle_error "Creating destination directory"
    shopt -s dotglob  # Enable copying hidden files
    cp -R ../lksdiycc2024/* /var/www/lksdiycc2024/ || handle_error "Copying Laravel project"
    validate_command "Copying Laravel project"
else
    handle_error "Source folder '../lksdiycc2024' not found"
fi
